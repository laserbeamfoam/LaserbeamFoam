...
// Lagrangian particle cloud used to deliver feedstock into the molten pool
...
int main(int argc, char *argv[])
{
    ...
    while (runTime.run())
    {
        ...
        // Evolve the particle cloud and compute melting sources
        // (alphaMeltSource, particleEnthalpySource, massSource).
        // Updates alpha1 directly with the melt contribution.
        #include "ParticleEvolution.H"

        // --- Pressure-velocity PIMPLE corrector loop
        while (pimple.loop())
        {
            // Required before any operation that moves the mesh or remaps
            // fields under the parcels, so the cloud can re-find cells.
